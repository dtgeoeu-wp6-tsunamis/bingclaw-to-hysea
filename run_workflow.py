...
scen_input_path = os.path.join(io_bingclaw_path, 'scenarios_input')
scen_output_path = os.path.join(io_bingclaw_path, 'scenarios_output')
dockerimage_name = 'ngiacr.azurecr.io/bingclaw:latest'  # Name of BingClaw docker image
...
donor = 'bingclaw'
donor_output = os.path.join(scen_output_path, scenario) # Bingclaw scenario output directory
...
filter = 'kajiura'                              # Filter for deformation data (kajiura / none)
...
# Run BingClaw
if (do_run_bingclaw):
    run_bingclaw(io_bingclaw_path, scen_input_path, scen_output_path, bathymetry, scenario, dockerimage_name)
else:
...
if (do_run_interface_module):
    run_interface_module(io_intmod_path, donor, donor_output, bathy_file, resolution, filter, casename)
else:
